  ...
  me.addListener("catchRemoteImage", function() {
    ...
    var remoteImages = [],
      ...
      test = function(src, urls) {
        // TODO: my: 打印 log
        console.log(
          'UE.plugins["catchremoteimage"] fn(test):',
          (src.indexOf(location.host) != -1 || /(^\.)|(^\/)/.test(src)),
        );
    ...
    for (var i = 0, ci; (ci = imgs[i++]); ) {
      ...
      if(ci.nodeName == "IMG"){
        ...
        // TODO: my: 打印 log
        console.log(
          'UE.plugins["catchremoteimage"]:',
          (/^(https?|ftp):/i.test(src) && !test(src, catcherLocalDomain)),
          src,
          catcherLocalDomain,
        );
